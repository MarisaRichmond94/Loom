...
// Canvas node-graph animation, shown while the reviewer makes its first pass
// (KAN-22). Eight chunk nodes orbit a pulsing hub, lighting one by one with
// pulses travelling the edges, then the cycle resets.
//
// PORTED VERBATIM from WriteAI's ui/SyncAnimation.tsx — itself from the
// author's rag_book_sync_nodes_only.html prototype. Loom already had a third
// copy of this, inlined as a document.write string in useWriteAiReview, to
// paint the tab that opened while the canon export ran. That splash is what
// the writer recognised and asked to reuse here.
//
// Copied rather than retyped so the motion is identical. It is now in two
// repos; if it changes, change both. A shared package is not worth it for one
// animation, but the duplication should be a deliberate memory, not a
// surprise.
//
// Light/dark is detected from the DOM rather than passed in: the panel sits
// inside the author layout's `.light-body` wrapper, which the chapter page
// itself has no handle on. Read once on mount — light mode toggling midway
// through a review is not worth an observer for.
...
export default function ReviewAnimation({ width = 190 }: Props) {
  ...
  useEffect(() => {
    const isLight = !!canvasRef.current?.closest(".light-body");
    setDark(!isLight);
  }, []);
  ...
  return (
    <canvas
      ...
      aria-label="Reading the chapter"
    />
  );
}
